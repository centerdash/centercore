...
export default async function handler(req: FastifyRequest<{ Body: Body }>, rep: FastifyReply) {
    ...
    if(req.body.levelID == '0') {
        ...
        const q = await query("INSERT INTO levels (userName, authorID, timestamp, name, description, version, length, song, customSong, objects, password, original, twoPlayer, coins, requestedStars, unlisted, extraString, levelInfo, ldm) VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
            req.body.userName,
            req.body.accountID,
            getTimestamp(),
            req.body.levelName,
        ...
        await query("UPDATE levels SET userName=?, authorID=?, updateTimestamp=?, name=?, description=?, version=?, length=?, song=?, customSong=?, objects=?, password=?, original=?, twoPlayer=?, coins=?, requestedStars=?, unlisted=?, extraString=?, levelInfo=?, ldm=? WHERE levelID = ? LIMIT 1", [
            req.body.userName,
            req.body.accountID,
            getTimestamp(),
            req.body.levelName,
