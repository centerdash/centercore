...
export default async function handler(req: FastifyRequest<{ Body: Body }>, rep: FastifyReply) {
    ...
    if(req.body.accountID && req.body.gjp && req.body.accountID == req.body.targetAccountID) {
        ...
        const q7 = await query("SELECT count(*) FROM friends WHERE (user1 = ? AND user2 = ?) OR (user1 = ? AND user2 = ?)", [req.body.accountID, req.body.targetAccountID, req.body.targetAccountID, req.body.accountID])
