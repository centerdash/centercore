...
export default function handler(req: FastifyRequest<{ Body: Body }>, rep: FastifyReply) {
    ...
    if(req.body.getSent == 0) {
        db.query("SELECT * FROM friend_reqs LEFT JOIN accounts ON friend_reqs.fromID = accounts.accountID WHERE toID = ? LIMIT 10 OFFSET ?", [req.body.accountID, offset], (err, q) => {
            let out = ''
